...
- **Free**: 0.5 RPS burst, 0.01 RPS sustained (1,000 req/day)
- **Starter**: 2 RPS burst, 0.1 RPS sustained (10,000 req/day)
- **Pro**: 5 RPS burst, 0.5 RPS sustained (50,000 req/day)
- **Enterprise**: Custom RPS (typically 50+ RPS)
...
- **Monthly Allocations**:
  - Free: 0.5 RPS burst & 10,000 CUs/month (10 MB of payload data)
  - Starter: 2 RPS burst & 1,000,000 CUs/month (1 GB of payload data)
  - Pro: 5 RPS burst & 10,000,000 CUs/month (10 GB of payload data)
...
interface IJobRegistry {
    ...
    function confirmJob(bytes32 jobId, bytes32 resultHash) external;
    function getJobConfirmation(bytes32 jobId)
        external view returns (bool confirmed, bytes32 resultHash, uint256 timestamp);
}
```
...
```
Free Tier:
  - 10,000 CUs/month included (10 MB)
...
interface IJobRegistry {
    ...
    // Core functions
    function confirmJob(bytes32 jobId, bytes32 resultHash) external;
    function batchConfirmJobs(bytes32[] calldata jobIds, bytes32[] calldata resultHashes) external;
    function getJobConfirmation(bytes32 jobId) external view
        returns (bool confirmed, bytes32 resultHash, uint256 timestamp);
}
```
